...
public class UserProcess {
    ...
    private int handleCreate(int index){
        String name = readVirtualMemoryString(index, stringMaxLength);
    ...
    private int handleOpen(int index){
        String name = readVirtualMemoryString(index, stringMaxLength);
        if (name == null) return -1;
    ...
    private int handleRead(int index, int buffer, int length){
        OpenFile file = descriptorManager.get(index);
        ...
        int val = file.read(space, 0, length);
        
        if (val == -1) return -1;
        
        return writeVirtualMemory(buffer, space, 0, val);
        
    }
    
    private int handleWrite(int index, int buffer, int length){
        OpenFile file = descriptorManager.get(index);
        ...
        int val = readVirtualMemory(buffer,read, 0, length);
        
        return file.write(read, 0, val);
        
    }
    
    private int handleClose(int index){
        return descriptorManager.close(index);
    }
    
    private int handleUnlink(int index){
        String name = readVirtualMemoryString(index, stringMaxLength);
        
        if (name == null) return -1;
        if (!UserKernel.fileSystem.remove(name)) return -1;
    ...
    public class DescriptorManager {
        ...
        protected OpenFile list[] = new OpenFile[length];
